...
"rhel-8.2" | "rhel-8.3" | "rhel-8.4")
  printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
  printf -- 'Installing the dependencies for HTTP server from repository \n' |& tee -a "$LOG_FILE"
  sudo yum install -y --skip-broken git openssl openssl-devel python2 gcc libtool autoconf make pcre pcre-devel libxml2 libxml2-devel expat-devel which wget tar |& tee -a "$LOG_FILE"
  configureAndInstall |& tee -a "$LOG_FILE"
  ;;
