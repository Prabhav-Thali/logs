...
function prepare() {
        ...
        if [[ "$JAVA_PROVIDED" != "AdoptJDK11_openj9" && "$JAVA_PROVIDED" != "AdoptJDK11_hotspot" && "$JAVA_PROVIDED" != "OpenJDK11" ]]; then
                err "$JAVA_PROVIDED is not supported, Please use valid java from {AdoptJDK11_openj9, AdoptJDK11_hotspot, OpenJDK11} only"
                exit 1
        fi
...
function printHelp() {
        echo
        echo "Usage: "
        echo "  build_elasticsearch.sh  [-d debug] [-y install-without-confirmation] [-t install-with-tests] [-j Java to use from {AdoptJDK11_openj9, AdoptJDK11_hotspot, OpenJDK11}]"
...
function printSummary() {
        ...
        printf -- "Note: To set the Environmental Variable needed for Elasticsearch, please run: source $HOME/setenv.sh \n"
